import { ArrowLeftToLine, ArrowRightToLine, Check, Circle, CopyPlus, Database, Download, Edit3, FilePlus2, Gamepad2, Headphones, Redo2, Save, Settings, Star, Trash2, Undo2, Upload, X, Zap } from 'lucide-react'
...
import { quantizeSelectedNotes, snapTimeToGrid } from './editor/quantize-selection'
...
import { MobilePlayControls } from './game/MobilePlayControls'
...
import { readAudioFileDurationMs } from './storage/browser-audio-attachment'
import { getBrowserAudio, putBrowserAudio, requestPersistentBrowserStorage } from './storage/browser-audio-repository'
...
import { useBrowserAudioAttachment } from './storage/use-browser-audio-attachment'
import {
  collectionPercent,
  countNotesByLane,
  createLaneCounts,
...
function isMobileLaneControlTarget(target: EventTarget | null) {
  return target instanceof HTMLElement && Boolean(target.closest('.mobile-lane-pad'))
}
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'play' | 'editor'>(() => {
    const stored = readAppStorage('workspace-mode') ?? readAppStorage('active-tab')
    return stored === 'editor' ? 'editor' : 'play'
  })
  const [settingsOpen, setSettingsOpen] = useState(() => {
    const stored = readAppStorage('active-tab')
    return stored === 'config' || stored === 'debug' || stored === 'settings'
  })
  const [settingsTab, setSettingsTab] = useState<'general' | 'debug'>(() => {
    const stored = readAppStorage('settings-tab') ?? readAppStorage('active-tab')
    return stored === 'debug' ? 'debug' : 'general'
  })
  const [editorSidebarTab, setEditorSidebarTab] = useState<'actions' | 'feedback'>('actions')
  ...
  const {
    ...
    deleteRun: deletePlayRun,
  } = usePlayRunHistory({ songId: importedSong?.id, beatmap, bpm, beatOffsetMs, enabled: !settingsOpen && activeTab === 'play', repository: runHistoryRepository })
  ...
  useEffect(() => { localStorage.setItem(storageKey('active-tab'), settingsOpen ? 'settings' : activeTab) }, [activeTab, settingsOpen])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setAudioAttachmentTargetId(song.id)
    void loadImport(song)
  }, [importedSong, loadImport, savedImports, setAudioAttachmentTargetId])
  ...
  const parry = useCallback((lane: BeatmapNote['lane'] = 'mid') => {
    ...
    recordRunJudgements([{ attack: target, grade: result.grade, deltaMs: result.deltaMs }], audioRef.current ? audioRef.current.currentTime * 1000 : target.noteTimeMs ?? 0)
    if (result.success) {
  ...
  }, [activeAttacks, recordRunJudgements, tuning])

  const pressPlayLane = useCallback((lane: Lane) => {
    setPadTriggers((triggers) => ({ ...triggers, [lane]: performance.now() }))
    setHeldPlayLanes((held) => new Set(held).add(lane))
    parry(lane)
  }, [parry])

  const releasePlayLane = useCallback((lane: Lane) => {
    setHeldPlayLanes((held) => {
      if (!held.has(lane)) return held
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (settingsOpen || isMobileLaneControlTarget(event.target)) return
  ...
  }, [activeTab, armedLanes, isRecording, keyLane, nextAttack, parry, quantize, quantizeTime, settingsOpen, tapMode])
  ...
  useEffect(() => {
    ...
    const pollGamepad = () => {
      ...
      if (!gamepad || settingsOpen) return
      ...
      pressed.forEach((buttonIndex) => {
        ...
        else if (activeTab === 'play') parry(lane)
      })
  ...
  }, [activeTab, armedLanes, gamepadLane, isRecording, parry, quantize, quantizeTime, settingsOpen, tapMode])

  useEffect(() => {
    if (settingsOpen || activeTab !== 'play' || !isSongPlaying) return
    const timer = window.setInterval(() => {
      const now = performance.now()
      const expired = activeAttacks.filter((attack) => !attack.holdStarted && !attack.initialMissed && now >= attack.impactMs + tuning.parryWindowMs)
      if (expired.length > 0) {
        recordRunJudgements(expired.map((attack) => ({ attack, grade: 'miss', deltaMs: null })), audioRef.current ? audioRef.current.currentTime * 1000 : expired[0]?.noteTimeMs ?? 0)
      ...
      const remainingAttacks = activeAttacks
        .map((attack) => expiredIds.has(attack.id) ? { ...attack, initialMissed: true } : attack)
        .filter((attack) => now < attack.impactMs + ((attack.durationMs ?? 0) > 0 ? (attack.durationMs ?? 0) : attack.initialMissed || expiredIds.has(attack.id) ? missedProjectileLingerMs : tuning.parryWindowMs))
      if (remainingAttacks.length !== activeAttacks.length || expired.length > 0) setActiveAttacks(remainingAttacks)
    }, 100)
    return () => window.clearInterval(timer)
  }, [activeAttacks, activeTab, isSongPlaying, recordRunJudgements, settingsOpen, tuning.parryWindowMs])
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      setActiveAttacks((attacks) => {
        ...
        return [...attacks, ...attacksToAdd.map(({ note, timeUntilImpactMs, scheduleKey }) => makeBeatmapAttack(timeUntilImpactMs, note.lane, note.durationMs, note.id, scheduleKey, note.strength, syncopationAmount(note.impactTimeMs, bpm, beatOffsetMs), note.impactTimeMs))]
  ...
  const importLocalAudio = useCallback(async (file: File) => {
    ...
    try {
      let browserStorageIsPersistent = true
      if (useCompanion) {
        ...
        browserStorageIsPersistent = await requestPersistentBrowserStorage()
      }
      setImportStatus(browserStorageIsPersistent
        ? 'Local audio attached. Add timing and custom notes in the editor.'
        : 'Local audio attached. Keep the original file because this browser may clear stored audio.')
  ...
  const importSharedBundle = useCallback(async (file: File) => {
    try {
      ...
      await loadImports()
      setAudioAttachmentTargetId(bundle.songs[0].id)
      setImportStatus(`Imported ${bundle.title}. Attach the matching local audio file to play it on this device.`)
    } catch (error) {
      setImportStatus(error instanceof Error ? `Share import failed: ${error.message}` : 'Share import failed')
    }
  }, [loadImports, packageRepository, setAudioAttachmentTargetId])
  ...
  const seekSong = useCallback((timeMs: number) => {
    ...
    resetGameplayPlayback()
    if (isSongPlaying) beginPlayRun(nextTimeMs)
  }, [beatmap?.durationMs, beginPlayRun, finishPlayRun, importedSong?.durationMs, isSongPlaying, resetGameplayPlayback])
  ...
  const playSong = useCallback(() => {
    resetGameplayPlayback()
    beginPlayRun(audioRef.current ? audioRef.current.currentTime * 1000 : songTimeMs)
    void audioRef.current?.play()
  }, [beginPlayRun, resetGameplayPlayback, songTimeMs])
  ...
  const restartSong = useCallback((fromLoopStart = false) => {
    ...
    setLastResult(null)
    setLastAutoMiss(false)
    beginPlayRun(restartMs)
    void audio.play()
  }, [beginPlayRun, finishPlayRun, loopMarkers.startMs, resetGameplayPlayback])
  ...
  const quantizeSelection = useCallback(() => {
    if (selectedNoteIds.size === 0 || !Number.isFinite(gridMs) || gridMs <= 0) return
    const hasTimingChanges = selectedNotes.some((note) => Math.abs(snapTimeToGrid(note.impactTimeMs, beatOffsetMs, gridMs) - note.impactTimeMs) > 0.001)
    if (!hasTimingChanges) return
    checkpointEditor()
    const applyQuantize = (notes: BeatmapNote[]) => quantizeSelectedNotes(notes, selectedNoteIds, beatOffsetMs, gridMs)
    setBeatmap((current) => current ? { ...current, notes: applyQuantize(current.notes) } : current)
    setRecordedNotes(applyQuantize)
    selectedNoteIds.forEach((id) => scheduledNoteIds.current.delete(id))
  }, [beatOffsetMs, checkpointEditor, gridMs, selectedNoteIds, selectedNotes])
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (settingsOpen || activeTab !== 'editor' || isEditingTarget(event.target)) return
      if (event.code === 'Space') {
  ...
  }, [activeTab, centerTimelineOnPlayhead, copiedNotes.length, copySelection, deleteSelection, handleLoopRulerClick, isSongPlaying, pasteSelectionAtPlayhead, pauseSong, playSong, redoEditor, restartSong, selectedNoteIds, setTimelineZoomPreservingPlayhead, settingsOpen, songTimeMs, undoEditor, zoomOptions])
  ...
  return (
    <main className={activeTab === 'editor' ? 'editing-layout' : 'play-layout'}>
      {settingsOpen && <section className="settings-page" aria-label="Settings">
        <div className="settings-page__shell">
          <header className="settings-page__header">
            <Tabs value={settingsTab} onValueChange={(value) => setSettingsTab(value as 'general' | 'debug')} className="settings-tabs"><TabsList className="settings-tabs__list"><TabsTrigger value="general">General</TabsTrigger><TabsTrigger value="debug">Debug</TabsTrigger></TabsList></Tabs>
            <div className="settings-page__title-row">
              <h1>Settings</h1>
              <Button type="button" variant="ghost" size="icon" onClick={() => setSettingsOpen(false)} tooltip="Close settings" aria-label="Close settings"><X /></Button>
            </div>
          </header>
          ...
          <ConfigSection className="import-card" icon={<Headphones />} title="Import audio" description="Choose a file in any browser. The optional companion additionally enables YouTube imports and local caching." status={<Badge tone={companionStatus === 'paired' ? 'success' : companionStatus === 'available' || companionStatus === 'connecting' ? 'warning' : companionStatus === 'permission-blocked' || companionStatus === 'unavailable' ? 'danger' : 'muted'}>{companionStatusLabel[companionStatus]}</Badge>}>
            <div className="import-card__content">
              ...
              <input ref={localAudioInputRef} type="file" accept="audio/*,.mp3,.m4a,.aac,.wav,.ogg,.flac" hidden onChange={(event) => { const file = event.currentTarget.files?.[0]; if (file) void importLocalAudio(file); event.currentTarget.value = '' }} />
      ...
      <aside className="panel" inert={settingsOpen}>
        <div className="panel-hero"><div className="panel-brand"><img src="/beat-fiend-logo.png" alt="" /><h1>Beat Fiend</h1></div></div>
        <div className="workspace-nav">
          <button type="button" className="mode-toggle" role="switch" aria-checked={activeTab === 'editor'} onClick={() => setActiveTab((mode) => mode === 'play' ? 'editor' : 'play')}>
            <span className={activeTab === 'play' ? 'mode-toggle__label mode-toggle__label--active' : 'mode-toggle__label'}><Gamepad2 />Play</span>
            <span className="mode-toggle__track" aria-hidden="true"><span className="mode-toggle__thumb" /></span>
            <span className={activeTab === 'editor' ? 'mode-toggle__label mode-toggle__label--active' : 'mode-toggle__label'}><Edit3 />Edit</span>
          </button>
          <Button type="button" variant="ghost" size="icon" className="settings-button" onClick={() => { audioRef.current?.pause(); if (isRecording) stopRecording(); setSettingsOpen(true) }} tooltip="Open settings" aria-label="Open settings"><Settings /></Button>
        </div>
        ...
        {activeTab === 'editor' && <>
          {beatmapControls}
          <Tabs value={editorSidebarTab} onValueChange={(value) => setEditorSidebarTab(value as 'actions' | 'feedback')} className="editor-sidebar-tabs">
            <TabsList className="editor-sidebar-tabs__list" aria-label="Editor sidebar panels">
              <TabsTrigger value="actions">Actions</TabsTrigger>
              <TabsTrigger value="feedback">Run feedback</TabsTrigger>
            </TabsList>
            <TabsContent value="actions">{beatmapActions}</TabsContent>
            <TabsContent value="feedback"><RunFeedbackSummaryCard summary={feedbackSummary} lastRunSummary={lastRunFeedbackSummary} lastRun={lastPlayRun} showLastRunOnly={showLastRunOnly} onShowLastRunOnlyChange={setShowLastRunOnly} onDiscardRun={deletePlayRun} /></TabsContent>
          </Tabs>
        </>}
