import { ArrowLeftToLine, ArrowRightToLine, Check, Circle, CopyPlus, Database, Download, Edit3, FilePlus2, Gamepad2, Headphones, Redo2, Save, Star, Trash2, Undo2, Upload, X } from 'lucide-react'
...
function App() {
  ...
  const parry = useCallback((lane: BeatmapNote['lane'] = 'mid') => {
    ...
    const nextFeedback = { id: Math.random(), kind, startedAtMs: inputTimeMs, lane, deltaMs: result.deltaMs }
    ...
    const timer = window.setInterval(() => {
      ...
      setActiveAttacks((attacks) => {
        ...
        if (isSongPlaying && expired.length > 0) {
          ...
          const missedFeedback = expired.map((attack) => ({ id: Math.random(), kind: 'miss' as const, startedAtMs: now, lane: attack.lane }))
